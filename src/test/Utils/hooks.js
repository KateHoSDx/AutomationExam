"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
Object.defineProperty(exports, "__esModule", { value: true });
const test_1 = require("@playwright/test");
const cucumber_1 = require("@cucumber/cucumber");
const pageFixtures_1 = require("./pageFixtures");
let browser;
let context;
let page;
(0, cucumber_1.BeforeAll)(function () {
    return __awaiter(this, void 0, void 0, function* () {
        browser = yield test_1.chromium.launch({ headless: false });
        context = yield browser.newContext();
        page = yield context.newPage();
        pageFixtures_1.pageFixtures.page = yield page;
    });
});
(0, cucumber_1.AfterAll)(function () {
    return __awaiter(this, void 0, void 0, function* () {
        yield pageFixtures_1.pageFixtures.page.close();
        yield context.close();
    });
});
(0, cucumber_1.After)(function (_a) {
    return __awaiter(this, arguments, void 0, function* ({ pickle, result }) {
        if ((result === null || result === void 0 ? void 0 : result.status) == cucumber_1.Status.FAILED) {
            const images = yield pageFixtures_1.pageFixtures.page.screenshot({ path: `./test-result/screenshot/${pickle.name}.png`, type: 'png' });
            yield this.attach(images, "image/png");
        }
    });
});
